import React from 'react'
import { useDispatch } from 'react-redux'
import { deleteCart, incrementQuantity } from '../state/cart.actions';

function Controls({ quantity, cardId }) {
  const dispatch = useDispatch();
  
  return (
    <div className="cartItem_actions">
      <div className="cartItem_count">
        <div className="cartItem_button">-</div>
        <div className="cartItem_number">{quantity}</div>
        <div 
          className="cartItem_button"
          onClick={()=> {
            dispatch(incrementQuantity(cardId))
          }}
        >
          +
        </div>
      </div>
      <div 
        className="carItem_remove"
        onClick={() => {
          dispatch(deleteCart(cardId))
        }}
      >
        <img
          alt=""
          src="data:image/png;base64,"
        />
      </div>
    </div>
  )
}

export default Controls